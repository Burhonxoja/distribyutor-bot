...
async def di_loc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Save to Sheets with error handling
    row = [
        dokon_id, nom, adr, mchj, tel1, tel2, sotuvchi,
        str(user_id), user_name, lat, lng, channel_msg_id, sana
    ]
    
    logger.info(f"DEBUG: Saving shop data:")
    logger.info(f"  dokon_id: {dokon_id}")
    ...
    try:
        ...
        if result:
            logger.info(f"✅ Shop saved successfully: {dokon_id}")
            await update.message.reply_text(
            ...
            logger.error(f"❌ db_add returned False for shop: {dokon_id}")
            await update.message.reply_text("❌ Xatolik yuz berdi. Qaytadan urinib ko'ring.")
    except Exception as e:
        logger.error(f"❌ Exception while saving shop {dokon_id}: {e}")
    ...
    for shop in selected_shops:
        ...
        for idx, cp in enumerate(custom_prices):
            if (str(cp.get('Dist_ID')) == str(user_id) and 
                ...
                # Update
                db_update_row("Mahsulotlar_Maxsus_Narx", idx + 2, 7, price)  # Sotish_Narxi column
                exists = True
                break
